...
from config import (
    ...
    MOOD_EMOJIS, EVENT_MOOD_TRIGGERS, GOBLIN_DISTRUST,
)
...
class AgentMood:
    ...
    _accumulated_trauma: float = 0.0  # накопленная травма — замедляет восстановление стресса/страха
    ...
    def apply_event(self, event_text: str, personality_type: 'PersonalityType',
        ...
        for category, data in EVENT_MOOD_TRIGGERS.items():
            ...
            if any(kw in event_lower for kw in keywords):
                ...
                # Гарантированный минимум реакции на опасность —
                # даже самый спокойный агент (N:20) должен испугаться бандитов
                if category in ('danger', 'sickness'):
                    ...
                    # Счастье не может оставаться высоким при опасности
        ...
        if not effects_applied:
            ...
            # Накопление травмы при опасных/негативных событиях
            if any(cat in ('danger', 'loss', 'sickness') for cat, data in EVENT_MOOD_TRIGGERS.items()
                ...
                # Сдвигаем baseline стресса и страха вверх (среда СТАНОВИТСЯ опаснее)
                ...
                # Немного снижаем baseline счастья
    ...
    def decay_toward_baseline(self, big_five: 'BigFiveTraits' = None):
        ...
        # Травма замедляет восстановление стресса и страха
        trauma_slowdown = 1.0 - self._accumulated_trauma * 0.6  # при trauma=1.0 → восстановление на 60% медленнее
        ...
        s_rate = max(0.005, s_rate)  # снижен минимум для замедления
        ...
        f_rate = max(0.005, f_rate)  # снижен минимум для замедления
        self.fear += (self._baseline_fear - self.fear) * f_rate

        # Медленное затухание самой травмы (но очень медленно — ~50 тиков для полного восстановления)
