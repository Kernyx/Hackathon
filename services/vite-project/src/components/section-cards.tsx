...
const extractTraits = (res: any) => {
  if (res.traits && typeof res.traits === 'object') return res.traits;
...
// Нормализация данных из API в формат UI
const normalizeAgent = (res: any): AgentData => {
  // ← ИСПРАВЛЕНО: Маппим роль из API enum в UI роль
  const rawRole = res.role || res.personalityType || "INDIVIDUAL";
  const mappedRole = API_ENUM_TO_UI_ROLE[rawRole] || rawRole;

  return {
    id: res.id,
    name: res.name || res.username || "Без имени",
    avatarSeed: res.avatarSeed || res.photo || res.photoLink || "Alex",
    age: res.age || "",
    male: res.male ?? res.isMale ?? true,
    role: mappedRole,
    interests: res.interests || "",
    traits: extractTraits(res),
    ownerId: res.ownerId || res.userId,
    isSynced: true,
...
export function SectionCards({ externalAgents }: { externalAgents?: any[] }) {
  ...
  // Синхронизация с externalAgents при любом изменении
  useEffect(() => {
    if (externalAgents && externalAgents.length >= 0) {
      const formatted = externalAgents.map(normalizeAgent);
      setAgents(formatted);
    }
  }, [externalAgents]);
  ...
  const handleDeleteRequest = (e: MouseEvent, id: string) => {
    ...
    const timerId = window.setTimeout(() => {
      if (pendingDeleteRef.current.has(id)) {
        AiAgentServiceService.deleteAiAgentAgents(id)
          .then(() => {
        ...
        {agents.map((agent) => {
          ...
          return (
            <Card
              ...
              `}
              onClick={() => {
                if (!isDeleting) setSelectedAgent(agent);
              }}
            >
              {isDeleting && (
                <div className="absolute inset-0 z-10 flex items-center justify-between px-4 bg-background/90 backdrop-blur-[2px] animate-in fade-in duration-300">
                  <span className="text-xs font-medium text-foreground">Агент будет удален...</span>
                  <Button
                    variant="outline"
                    size="sm"
